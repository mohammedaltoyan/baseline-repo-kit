...
- Keep PR author and reviewer identities separate when approvals are required:
  ...
  - Baseline Auto-PR uses the GitHub Actions bot by default (`github-actions[bot]` / `app/github-actions`); bootstrap enables the required Actions permission in policy (`github.workflow_permissions.can_approve_pull_request_reviews=true`).
